...
(* Scoped generation for the in-process LSP pipeline: when set to
   [Some (a, b)] (line numbers of the current document version), only
   the theorems intersecting that window have their proofs generated;
   the others contribute their context and keep their parsed proofs.
   The LSP owns supplying the missing obligations (it carries them from
...
let modctx_of_string ~(content : string) ~(filename : string) ~loader_paths ~prefer_stdlib : (modctx * Module.T.mule, string option * string) result =
    let parse_it () =
        ...
        (* Probe (TLAPM_LSP_PHASES=1): stage attribution of the
           in-process pipeline — main-file parse, dependency loading,
           dependency elaboration, main-module elaboration+generation.
           Inert without the environment variable. *)
        let phases = Sys.getenv_opt "TLAPM_LSP_PHASES" <> None in
        let tick = ref (if phases then Unix.gettimeofday () else 0.) in
        let lap name =
            if phases then begin
                let now = Unix.gettimeofday () in
                Printf.eprintf "[LSP_PHASES]   %s=%.2fs\n%!"
                    name (now -. !tick);
                tick := now
            end in
        ...
        let mcx, mule = Std.finally Clocks.stop (List.fold_left (fun (mcx, found) m ->
            ...
            if phases then begin
                ...
                if is_main then t_main := !t_main +. dt
                else t_deps := !t_deps +. dt
            end;
            match is_main with
            | true -> (mcx, Some m)
            | false -> (mcx, found)
        ...
        if phases then
            Printf.eprintf
                "[LSP_PHASES]   elab_deps=%.2fs elab_main(+gen)=%.2fs\n%!"
                !t_deps !t_main;
        match mule with
