...
| L5 | Under `--printallobs` (always set by the LSP), a fingerprint *hit* still pays a duplicate `expand_defs` + normalize just to print the message — `toolbox.ml` contains a verbatim copy of the expansion code. **Measured worse than supposed**: the copy kept the *quadratic* pre-phase-1 algorithm, making a fully-cached warm run 4× slower than reproving; fixed by the same single-pass rewrite (see PHASE4.md «4c/4d»), printed text byte-identical | `src/backend/toolbox.ml:100-133` (dup of `src/backend/prep.ml:38-54`), called from `prep.ml:1550,1583` |
