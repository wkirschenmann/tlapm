...
<p>Both kinds are proof trees four or five levels deep: %d steps at depth&nbsp;four,
%d at depth&nbsp;five. Depth is not decoration. tlapm emits one obligation per leaf,
and every step's statement joins the context of its later siblings and of everything
nested beneath it, so a tree of depth five costs quite differently from the same
leaves laid out flat. The whole file is generated by
<code>harness/gen_l2proofs.py</code> from the lemma index with no randomness, so
regenerating is byte-identical and a diff means the generator changed.</p>
<p>The invariants underneath are written in the shape real refinement invariants take
&mdash; typing conjuncts over function spaces, quantified implications, negations,
sequence lengths, cardinalities, nested application &mdash; and level&nbsp;1's are
compositional, naming level&nbsp;0's properties through the instance prefix as well as
constraining its own state. That is deliberate: the quantity this corpus is about is
what a context of definitions costs, and a one-line predicate makes a definition that
costs nothing to carry.</p>""" % (
